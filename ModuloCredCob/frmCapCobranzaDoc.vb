...
#Region "Botón Aceptar P. Saldo a favor"
    Private Sub btnAceptarSF_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) 'Handles btnAceptar.Click
        Dim PagoExcesoTPV As Decimal = 0
        Dim oConfig As New SigaMetClasses.cConfig(GLOBAL_Modulo, CShort(GLOBAL_Empresa), GLOBAL_Sucursal)
        Dim ReglaTPVActiva As Boolean = False

        Try
            PagoExcesoTPV = CType(oConfig.Parametros("PagoExcesoTPV"), Decimal)
            ReglaTPVActiva = CType(oConfig.Parametros("ReglaTPVActiva"), Boolean)
        Catch ex As Exception
            PagoExcesoTPV = 0
...
#End Region
    ...
    Private Sub ConsultaDocumentosCliente()
        ...
        If Not ValidaClientesAnticipo(txtCliente.Text.Trim) Then
            MessageBox.Show("¡ El número de cliente no es valido para un Anticipo, el cliente deberá ser un cliente emparentado!", Titulo, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If
    ...
    Function ValidaClientesAnticipo(Numcliente As String) As Boolean
        Dim oCliente As New SigaMetClasses.cCliente
        Dim dtCtesAnticipos As DataTable

        If TipoCobro = 21 Then
            dtCtesAnticipos = oCliente.ConsultaClientesAnticipos(Cliente)
            Dim dr As DataRow = dtCtesAnticipos.Select("cliente='" + Numcliente + "'").FirstOrDefault()
...
#End Region
    ...
    ''' <summary>
    ''' Función que valida pago en exceso TPV
    ''' </summary>
    ''' <param name="ImporteRestante"></param>
    ''' <returns></returns>
    Private Function ValidaPagoEnExcesoTPV(ImporteRestante As Decimal) As Boolean
        Dim oConfig As New SigaMetClasses.cConfig(GLOBAL_Modulo, CShort(GLOBAL_Empresa), GLOBAL_Sucursal)
        Dim PagoExcesoTPV As Decimal = 0
